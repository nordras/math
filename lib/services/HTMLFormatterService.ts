...
const HEADER_BORDER_SIZE = 3;
const CARD_BORDER_SIZE = 4;
const PROBLEM_FONT_SIZE = 20;
const TITLE_FONT_SIZE = 28;
const SCORE_NUMBER_FONT_SIZE = 32;

/**
 * Options for formatting grid problems
 */
export interface FormatGridOptions {
...
/**
 * Options for formatting contextual problems
 */
export interface FormatContextualOptions {
  /** Whether to include answer key */
  includeAnswerKey?: boolean;
  /** Student name to display */
  studentName?: string;
  ...
  <style>
    * {
    ...
    .header {
      text-align: center;
      margin-bottom: 30px;
      border-bottom: ${HEADER_BORDER_SIZE}px solid #ff6b9d;
      padding-bottom: 20px;
    }
    ...
    .problems-grid {
      display: grid;
      grid-template-columns: repeat(${DEFAULT_GRID_COLUMNS}, 1fr);
      gap: 15px 30px;
      margin-top: 30px;
    }
    ...
    .problem-expression {
      font-size: ${PROBLEM_FONT_SIZE}px;
  ...
  <div class="score-section" style="margin-top: 40px; padding: 20px; border: 2px solid #ff6b9d; border-radius: 12px; text-align: center;">
    ...
    <div style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
      <div style="background: #f0f0f0; padding: 15px 30px; border-radius: 8px;">
        ...
        <div style="font-size: ${SCORE_NUMBER_FONT_SIZE}px; font-weight: bold; color: #c084fc;">${stats.totalProblems || problems.length}</div>
      </div>
      <div style="background: #f0fdf4; padding: 15px 30px; border-radius: 8px;">
        <div style="font-size: 14px; color: #666;">Acertos</div>
        <div style="font-size: ${SCORE_NUMBER_FONT_SIZE}px; font-weight: bold; color: #36d399;">____</div>
      </div>
      <div style="background: #fff7ed; padding: 15px 30px; border-radius: 8px;">
        <div style="font-size: 14px; color: #666;">Minha Nota</div>
        <div style="font-size: ${SCORE_NUMBER_FONT_SIZE}px; font-weight: bold; color: #fb923c;">____</div>
      </div>
    </div>
 ...
 * @returns Formatted HTML string ready for rendering
 */
  ...
  <style>
    * {
    ...
    .problem-card {
      ...
      margin-bottom: 25px;
      box-shadow: 0 2px 8px rgba(0,0,0,0.1);
      border-left: ${CARD_BORDER_SIZE}px solid #ff6b9d;
    }
...
<body>
  ...
  <div class="score-section" style="margin-top: 40px; padding: 20px; border: 2px solid #c084fc; border-radius: 12px; text-align: center;">
    ...
    <div style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
      <div style="background: #f0f0f0; padding: 15px 30px; border-radius: 8px;">
        <div style="font-size: 14px; color: #666;">Total de Questões</div>
        <div style="font-size: ${SCORE_NUMBER_FONT_SIZE}px; font-weight: bold; color: #c084fc;">${contextualProblems.length}</div>
      </div>
      <div style="background: #f0fdf4; padding: 15px 30px; border-radius: 8px;">
        <div style="font-size: 14px; color: #666;">Acertos</div>
        <div style="font-size: ${SCORE_NUMBER_FONT_SIZE}px; font-weight: bold; color: #36d399;">____</div>
