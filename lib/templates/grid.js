...
class GridFormatter {
  ...
  generateTextGrid(problems) {
    ...
    problems.forEach((problem, index) => {
      const num = index + 1;
      const _letter = String.fromCharCode(65 + (index % 10)); // A-J
  ...
  generateHtmlGrid(problems) {
    ...
    let html =
      ...
      columns +
      ', 1fr); gap: 30px; text-align: center; font-family: monospace; font-size: 18px; margin: 20px 0;">\n\n';
    ...
    problems.forEach((problem) => {
      ...
      html +=
        '  <div style="border-top: 2px solid #333; margin: 5px 0; padding-top: 5px;">____</div>\n';
      html += '</div>\n\n';
    });
...
`;
  ...
  generateComplete(problems, stats, options = {}) {
    const { includeAnswerKey = false, includeStats = true, style = this.options.style } = options;
